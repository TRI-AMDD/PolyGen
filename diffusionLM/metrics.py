...
def extract_smiles(token, label):
    ...
    if label == "conditional":
        valid_token = split_token[start + 1: end - 1]
...
def extract_samples(df):
    df_select_high = []
    df_select_low = []
    for t in df["mol_smiles"]:
        if "[Ag]" in t.split(" "):
            df_select_high.append(t)
        elif "[Ac]" in t.split(" "):
            df_select_low.append(t)
            
    return pd.DataFrame(df_select_high, columns=["mol_smiles"]), pd.DataFrame(df_select_low, columns=["mol_smiles"])
