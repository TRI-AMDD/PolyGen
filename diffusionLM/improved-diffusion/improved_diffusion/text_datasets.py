...
def get_corpus_rocstory(data_args, model, image_size, padding_mode='block',
    ...
    if data_args.experiment_mode == 'lm':
        ...
        #        path = f'{data_args.roc_train}/htpmd_train.json'
        #    elif split == 'valid':
        #        print('loading form the VALID set')
        #        path = f'{data_args.roc_train}/htpmd_valid.json'
        #    else:
        ...
        if data_args.modality == 'roc':
            ...
            with open(path, 'r') as roc_reader:
                json_data = json.load(roc_reader)

            for row in json_data:
                word_lst = tokenizer.tokenize(row)
                sentence_lst.append(word_lst)
        ...
        if data_args.modality == 'roc-aug':
            ...
            if split == 'train':
                print('loading form the TRAIN set')
                path_lst = [f'{data_args.roc_train}/htpmd_train.json']
                ...
                print('loading form the VALID set')
                path_lst = [f'{data_args.roc_train}/htpmd_valid.json']
            else:
                assert False, "invalid split for ROC dataset"
